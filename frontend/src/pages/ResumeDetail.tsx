    ...
    <div className="space-y-4 text-sm">
      {(b.name || b.title || b.email) && (
        <div className="pb-4 border-b border-[var(--border)]">
          ...
          {b.title && (
            <div className="text-[var(--primary-strong)] text-sm">{b.title}</div>
          )}
          <div className="text-xs text-[var(--muted-foreground)] mt-1 flex flex-wrap gap-x-3 gap-y-1">
          ...
          <div>
            ...
            <ul className="space-y-1.5">
              {s.experience.slice(0, 5).map((e, i) => (
                <li key={i}>
                  <span className="text-[var(--foreground)] font-medium">{e.role}</span>
                  {e.company && (
        ...
        {s.education?.length > 0 && (
          <div>
            ...
            <ul className="space-y-1.5">
              {s.education.map((e, i) => (
                <li key={i}>
                  <span className="text-[var(--foreground)] font-medium">{e.degree}</span>
                  {e.school && (
                    <span className="text-[var(--muted-foreground)]"> · {e.school}</span>
                  )}
                </li>
              ))}
            </ul>
          ...
          <ul className="space-y-1.5">
            {s.projects.slice(0, 5).map((p, i) => (
              <li key={i}>
                ...
                {p.tech?.length > 0 && (
                  <span className="ml-2 text-[11px] text-[var(--primary-strong)]">
                    {p.tech.join(" · ")}
                  </span>
                )}
              </li>
