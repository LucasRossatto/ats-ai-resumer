...
export const resumesApi = {
  ...
  list: async (): Promise<ResumesListResponse> => {
    await mockDelay();
    return { resumes: listMockResumesShallow() };
  },

  // get: (id) => apiClient.get(`/resumes/${id}`).then((r) => r.data),
  get: async (id: string): Promise<ResumeGetResponse> => {
    await mockDelay();
  ...
  getVersion: async (id: string, versionId: string): Promise<ResumeVersionResponse> => {
    await mockDelay();
    const resume = findMockResume(id);
  ...
  //     .then((r) => r.data);
  // },
  upload: async (file: File, title?: string): Promise<ResumeUploadResponse> => {
    await mockDelay(800);
  ...
  remove: async (): Promise<OkResponse> => {
    await mockDelay();
    return { ok: true };
  },
  ...
  analyze: async (
    _id: string,
    { versionId }: { versionId?: string; targetRole?: string } = {}
  ): Promise<AnalysisResponse> => {
    await mockDelay(1200);
    const analysis =
      (versionId && mockAnalyses[versionId]) ||
      mockAnalyses[Object.keys(mockAnalyses)[0]];
    return { analysis };
  },

  // analyses: (id) => apiClient.get(`/resumes/${id}/analyses`).then((r) => r.data),
  analyses: async (id: string): Promise<AnalysesResponse> => {
    await mockDelay();
    const resume = findMockResume(id);
  ...
  analysisForVersion: async (_id: string, versionId: string): Promise<AnalysisResponse> => {
    await mockDelay();
  ...
  rewrite: async (
    id: string,
    { rewriteIds = [] }: { rewriteIds?: string[]; analysisId?: string } = {}
