...
import type { AuthResponse, OkResponse, User } from "@/types/api";
import type { ApiError } from "@/types/common";

export const authApi = {
  // register: (payload) => apiClient.post("/auth/register", payload).then((r) => r.data),
  register: async ({ name, email }: { name?: string; email?: string }): Promise<AuthResponse> => {
    await mockDelay();
  ...
  login: async ({ email }: { email?: string }): Promise<AuthResponse> => {
    await mockDelay();
  ...
  updateProfile: async (payload: Partial<User>): Promise<AuthResponse> => {
    await mockDelay();
